...
(function() {
  ...
  var load = function() {
    ...
    $("[data-role=\"lazy\"]").filter(":not(.loaded)").find(".lazy").Lazy({
    ...
    $("[data-role=\"lazyimg\"]").filter(":not(.loaded)").Lazy({
      scrollDirection: "vertical",
      enableThrottle: true,
      chainable: false,
      visibleOnly: true,
...
(function() {
	...
	new Vue({
		...
		mounted: function() {
			APP = this;
			var areas = [];
			APP["items"] = APP["all"].filter(function(item) {
				if (item["status"]) {
					item["img"] 	= COMMERCE["path"] + item["img"];
					item["plan"] 	= COMMERCE["path"] + item["plan"];
					areas.push(item["area"]);
					return item;
				}
			});
			areas = APP.unique(areas);
			areas = APP.sort(areas, "asc");
			APP["areas"] = areas;
			APP["selectedArea"] = areas[0];
		},
		watch: {
			selectedArea: function() {
				APP["imgLoaded"] = false;
				APP["flat"] = APP["items"].filter(function(item) {
					return item["area"]==APP["selectedArea"];
				})[0];
			},
			imgLoaded: function() {
				APP.initFancybox();
		...
		methods: {
			initFancybox: function() {
				$(APP["$el"]).find("[data-fancybox]").fancybox({
					hash: false,
			    protect: true,
...
(function() {
	...
	var $section 	= $("[data-role=\"contacts\"]");
	var $map 			= $("[data-role=\"map\"]");
	var $office		= $("[data-role=\"office\"]");
	...
	var pin 			= {
		src: 		$map.data("marker"),
		width: 	$map.data("sizes")[0],
		height: $map.data("sizes")[1],
		offset: $map.data("offset"),
	};

	var office = {
		latlng: $office.data("latlng"),
		src: 		$office.data("marker"),
		width: 	$office.data("sizes")[0],
		height: $office.data("sizes")[1],
		offset: $office.data("offset"),
	};
	...
	var load = function() {
		...
		DG.then(function() {
			...
			var officeIcon = DG.icon({
				iconUrl: office["src"],
				iconSize: [office["width"], office["height"]],
				iconAnchor: [office["width"]/2, office["height"]]
			});

			if (!$phone.is(":visible")) {
				DG.marker(office["latlng"], {
					icon: officeIcon,
					riseOnHover: true
				}).addTo(map);
			}
...
(function() {
	...
	var $groups = $form.find("[data-role=\"group\"]");
	var $name 	= $inps.filter("[name=\"name\"]");
	var $phone 	= $inps.filter("[name=\"phone\"]");


	$inps
		.on("focus", function() {
			$(document).trigger("inputEventFocus");
		})
		.on("blur", function() {
			$(document).trigger("inputEventBlur");
		});
	...
	var reset = function() {
		...
		$form
			...
			.find("[data-role=\"group\"]").removeClass("error").end()
			.find("[data-role=\"response\"]").hide().end()
			.find("[data-role=\"data\"]").show();
	};
	...
	var send = function() {
		...
		// setTimeout(function() {
			$form
				.removeClass("form--loading")
				.find("[data-role=\"data\"]").hide().end()
				.find("[data-role=\"response\"]").show();
		// }, 500);
		});
	};
	...
	var check = function() {
		...
		var enable = {
			name: 	name && name!="",
			phone: 	phone && phone!="" && !$phone.hasClass("incomplete")
		};

		$name.parents("[data-role=\"group\"]").removeClass("error").addClass(!enable["name"] ? "error" : "");
		$phone.parents("[data-role=\"group\"]").removeClass("error").addClass(!enable["phone"] ? "error" : "");

		if (!enable["phone"]) $phone.focus();
		if (!enable["name"]) $name.focus();

		if (enable["name"] && enable["phone"]) send();
	};
	...
	$inps.on("keyup change", function() {
		$(this).parents("[data-role=\"group\"]").removeClass("error");
	});
